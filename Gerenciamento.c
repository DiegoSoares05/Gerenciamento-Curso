...
/**                                                                 **/
/**   Diego soares de carvalho Junior            14652571           **/
/**                                                                 **/
/*********************************************************************/
...
/*
TURMA: tem por objetivo representar uma turma. E uma estrutura que possui cinco
campos: codigoDisciplina do tipo inteiro que corresponde ao codigo numerico da
disciplina, nomeDisciplina do tipo ponteiro para caracteres que aponta para o
nome da disciplina; totalVagas do tipo inteiro que contem o numero de vagas
oferecidas na turma; numAlunos do tipo inteiro que contem o numero atual de
alunos na turma; e nusps do tipo ponteiro para inteiro que aponta para um
arranjo que contem o numero USP de cada um dos alunos da turma.
*/
typedef struct {
...
/*
DESEMPENHO: tem por objetivo representar o desempenho de um aluno em uma turma.
Possui quatro campos: turma que e do tipo ponteiro para TURMA e aponta para a
turma a que o desempenho se refere; nota do tipo inteiro que contem a nota do
aluno na respectiva turma (de 0 [zero] a 100); frequencia do tipo inteiro que
contem a frequencia do aluno na respectiva turma (de 0 [zero] a 100); e status
do tipo char cujo valor sera 'M' para o aluno recem matriculado na turma e
podera ter outros dois valores possiveis dependendo da nota e da frequencia do
aluno ('A' para aprovado e 'R' para reprovado).
*/
typedef struct {
  TURMA *turma;
  int nota;
...
/*
ALUNO: tem por objetivo representar um aluno e seu historico escolar. Possui
cinco campos: nome do tipo ponteiro para caracteres que aponta para o nome do
aluno; nusp do tipo inteiro que representa o numero USP do aluno; historico do
tipo arranjo de DESEMPENHO que corresponde ao historico escolar do aluno, com
suas matriculas e desempenho nas turmas; turmasNoHistorico do tipo inteiro que
indica em quantas turmas o aluno ja se matriculou; e status do tipo char cujo
valor sera 'M' para o aluno que ainda nao conclui seu curso e podera ter outros
dois valores possiveis dependendo de seu desempenho no conjunto de turmas ('F'
...
/*
CURSO: tem por objetivo representar um curso que e composto por turmas e alunos.
Possui quatro campos: alunos do tipo arranjo de ponteiros para ALUNOs; numAlunos
do tipo inteiro que armazena a quantidade de alunos no curso; turma do tipo
arranjo de ponteiros para TURMAs; e numTurmas do tipo inteiro que armazena a
quantidade de turmas no curso.
*/
typedef struct {
...
bool cadastrarNota(ALUNO *aluno, int codigoDisciplina, int nota, int freq) {
  ...
  // verificando se aluno não possui historico nessa discipina 
  for (int i = 0; i < aluno->turmasNoHistorico; i++) {
    if (aluno->historico[i].turma->codigoDisciplina == codigoDisciplina &&
        aluno->historico[i].turma == NULL &&
        aluno->historico[i].status == 'M') {
      return false;
    } // efetuando o cadastro das notas e verificando status
    else{
...
void atualizarStatusAluno(ALUNO *aluno) {
  ...
  int aprov = 0; // numero de aprovações
  int repro = 0; // numero de reprovações

  for (int i = 0; i < aluno->turmasNoHistorico;
       i++) { // percorre o historico do aluno para ver o desempenho em cada
              // matéria, se for 'A' eu adiciono uma aprovação, se for 'R' eu
              // adiciono uma reprovação

    if (aluno->historico[i].status == 'A') { // atualiza o valor de aprovações
      aprov++;
    } else if (aluno->historico[i].status ==
               'R') { // atualiza o valor de reprovações
      repro++;
    }
  }
  if (aprov >= DISCIPLINAS_NECESSARIAS) { // aluno formado
    aluno->status = 'F';

  } else if ((MAX_TURMAS - repro) < DISCIPLINAS_NECESSARIAS) {
    aluno->status = 'J'; // aluno jubilado
  } else {
    aluno->status = 'M'; // aluno continua matriculado
  }
}
...
bool matricularAlunoEmTurma(ALUNO *aluno, TURMA *turma) {
  ...
  // efetuando as verificações

  if (aluno == NULL || turma == NULL) {
    return false;
  }

  // chamando função que atualiza o status do aluno, com esse aluno como
  // parametro
  atualizarStatusAluno(aluno);

  if (turma->numAlunos == turma->totalVagas || aluno->status != 'M' ||
      aluno->turmasNoHistorico == MAX_TURMAS) {
    return false;
  }

  // verificando se aluno está matriculado na turma
  for (int i = 0; i < turma->numAlunos; i++) {
    if (aluno->nusp == turma->nusps[i]) {
      return false;
    }
  }

  // verificando se está matriculado em outra turma da mesma disciplina ou ja
  // foi aprovado ou está matriculado

  for (int i = 0; i < aluno->turmasNoHistorico; i++) {
    if (aluno->historico[i].turma->codigoDisciplina ==
...
void imprimirDadosCurso(CURSO c1) {
  ...
  for (int t = 0; t < c1.numTurmas; t++) {
    ...
    printf("Turma: %s\tvagas: %i\tmatriculados: %i\n", turma->nomeDisciplina,
           turma->totalVagas, turma->numAlunos);
  }
  printf("#####################################################################"
         "##############\n");
}
...
int main() {
  ...
  ALUNO a7 = novoAluno("Diego \0", 14655);
  ...
  printf("\n\n*** Matriculando alunos nas turmas "
         "**************************************\n");
  if (matricularAlunoEmTurma(&a7, &t6))
    printf("15. OK\n");
  ...
  if (matricularAlunoEmTurma(&a1, &t3))
    ...
    printf("NOK\n");
  if (matricularAlunoEmTurma(&a3, &t2))
    printf("18. OK\n");
  else
    printf("NOK\n");
  ...
  if (matricularAlunoEmTurma(&a3, &t1))
    ...
    printf("NOK\n");
  if (matricularAlunoEmTurma(&a4, &t6))
    printf("22. OK\n");
  else
    printf("NOK\n");
  ...
  if (cadastrarNota(&a7, t6.codigoDisciplina, 100, 90))
    printf("23. OK\n");
  else
    printf("NOK\n");
  ...
  printf("\n\n*** Matriculando alunos nas turmas (novamente) "
         "**************************\n");
  if (matricularAlunoEmTurma(&a2, &t3))
    printf("28. OK\n");
  else
    printf("NOK\n");
  ...
  if (matricularAlunoEmTurma(&a1, &t5))
    ...
    printf("NOK\n");
  if (matricularAlunoEmTurma(&a3, &t6))
    printf("31. OK\n");
  else
    printf("NOK\n");
